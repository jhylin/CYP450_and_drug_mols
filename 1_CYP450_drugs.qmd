...
Draw.MolsToGridImage(
    ...
    highlightAtomLists=[m.GetSubstructMatch(all_3a4_mcs.queryMol) for m in mols_all]
    )
```
...
The idea came from a RDKit post by Paolo Tosco on, "[New MCS features in 2023.09.1](https://greglandrum.github.io/rdkit-blog/posts/2023-10-27-mcswhatsnew.html)" - the "Custom Python AtomCompare and BondCompare classes" section (other 3 sections were on degenerate MCSs, FinalMatchChecker and ShouldAcceptMCS - might be useful in some other cases)
...
all_3a4_ringMCS = rdFMCS.FindMCS(mols_all, params)

Draw.MolsToGridImage(
    mols_all, 
    subImgSize=(400, 400), 
    molsPerRow=3, 
    legends = list(df_3a4_all["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(all_3a4_ringMCS.queryMol) for m in mols_all]
    )
```
...
* When turning off CompleteRingsOnly, this allowed partial rings showed highlighted MCS 

* Similar output was generated when using ringMatchesRingOnly = True in FindMCS() for all of the CYP3A4 inhibitors earlier

<br>

**Non-ring systems**

Thinking about stereochemistry of the molecules, specifically for bonds:

e.g. MCSBondCompare() class

- CheckBondRingMatch()

- **CheckBondStereo()** - used in CompareOrderOutsideRings() class code below
...
- **MatchStereo** (to include bond stereo in comparison) - used in CompareOrderOutsideRings() class code below


```{python}
# Using CompareOrderOutsideRing class (for bonds) & CompareElementsOutsideRing class (for atoms) to see if MCS changes 

# Class code below were borrowed from RDKit blog post (https://greglandrum.github.io/rdkit-blog/posts/2023-10-27-mcswhatsnew.html), which explained the code very well already, particularly on why this was done as a custom "class" code and not a usual "function" one

## *might be nice to add further comments to explain the class code*

# Comparing bond orders outside ring systems using rdFMCS's MCSBondCompare
...
# Comparing atom elements outside ring systems using rdFMCS's MCSAtomCompare
...
all_3a4_orMCS = rdFMCS.FindMCS(mols_all, params_or)

Draw.MolsToGridImage(
    mols_all, 
    subImgSize=(400, 300), 
    molsPerRow=3, 
    legends = list(df_3a4_all["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(all_3a4_orMCS.queryMol) for m in mols_all]
    )
```

By using the suggested CompareOrderOutsideRings() and CompareElementsOutsideRings() classes, the MCS has indeed become larger, where a full ring was highlighted as the MCS.

<br>

[Another code example](https://gist.github.com/iwatobipen/6d8708d8c77c615cfffbb89409be730d) from iwatobipen below showed how to highlight molecular differences (which was opposite to highlighting only the MCS). Alternative URL link and more examples can be accessed via [RDKit Cookbook](https://rdkit.org/docs/Cookbook.html#highlight-molecule-differences).

Possible use cases of this might be when dealing with a large set of analogues with changes in R-groups or during large compound screening and search (just as a few examples only, not exhaustive). The main thing I could see from the code below that was to find the MCSs for the input molecules, then use SMARTS strings of the MCSs in order to find atoms not in the MCSs ([GetSubstructMatch()](https://rdkit.org/docs/source/rdkit.Chem.rdchem.html#rdkit.Chem.rdchem.Mol.GetSubstructMatch) part) then highlight that part of the molecules.
...
Draw.MolsToGridImage(
    ...
    subImgSize=(400, 300), 
    molsPerRow=2, 
    legends = list(df_2d6_s_inh_p["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(s2d6_mcs.queryMol) for m in mols_s2d6]
    )
```
...
Draw.MolsToGridImage(
    ...
    subImgSize=(400, 300), 
    molsPerRow=2, 
    legends = list(df_2d6_m_inh_p["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(m2d6_mcs.queryMol) for m in mols_m2d6]
    )
```
...
Draw.MolsToGridImage(
    ...
    subImgSize=(400, 300), 
    molsPerRow=2, 
    legends = list(df_2d6_all["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(all_2d6_mcs.queryMol) for m in mols_all_2d6]
    )
```
...
This post has become too long already so I'm going to stop here and gather some small findings if there are any.
...
* Many pharmaceuticals have different types of rings found in their chemical structures [@Taylor2014], so it is unsurprising that inhibiting CYP3A4 is quite common in approved drugs

* Might be interesting to compare the MCS found for CYP3A4 inhibitors with the MCS of CYP3A4 substrates

* It appeared that the difference between the MCS of moderate and strong CYP2D6 inhibitors was the alkyl chain attached to ring structures in strong CYP2D6 inhibitors, which was not found in the moderate CYP2D6 inhibitors

* As to the differences between CYP3A4 and 2D6 inhibitors, perhaps it was very subtle, as both groups seemed to show ring structures... Perhaps looking at CYP2D6 substrates will help (I'm only scratching the surface here maybe)

* It might also worth to spend more time to explore the CYP structures in relation to these inhibitors (this would be more of a structure-based drug discovery realm), which might reveal more insights 
