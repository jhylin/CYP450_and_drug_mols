...
Well, I've just found out a very strange thing in ChEMBL database, which is if I searched for "itraconazole" directly into the search field, only four entries appeared with ChEMBL IDs such as CHEMBL64391, CHEMBL22587, CHEMBL882 and CHEMBL5090785 appeared, and there was no ChEMBL224725. Then I noticed something that might be causing this discrepancy - a spelling error of itraconazole into "itraconzole", which was actually carried over into PubChem as well! [insert ![alt](link)] So to add this specially-spelled "itraconzole" into the dataframe, I did a quick fix which was literally adding the wrongly spelled name into the SQL query above when obtaining drug information through chembl_downloader.
