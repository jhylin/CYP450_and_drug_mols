...
Something about drug toxicology, or really as this post evolves, it's more about metabolism actually but also related to toxicity.

There seems to involve (in a very rough sense) 3 main types: *structural alerts* (in relation to known adverse effects or PAINs), *Human ether-a-go-go-related gene (hERG) potassium channel* and *cytochrome P450 enzymes* (CYP450, CYP isoenzymes, CYP or CYPs). There are most likely more as well (e.g. skin sensitisations, drug-induced liver injuries and more).

Recent commentary that mentions about "avoidome" - ref: "Structure is beauty, but not always truth" (cite: https://doi.org/10.1016/j.cell.2024.01.003) talks about DMPK-related proteins to avoid 
...
There are 5 main ones that are usually being investigated which are CYP3A4, 2D6, 1A2, 2C9 and 2C19, but there are in fact more than these 5 CYPs, other CYPS are CYP1A1, 2A6, 2B6, 2C8, 2E1, 2J2, 3A5 (note: amlodipine), 3A7 and 4F2 (cite Guengerich paper).
...
From ChEMBL 20, only 6 filters were present - http://chembl.blogspot.com/2015/02/chembl-20-released.html (it might appear that this blog post cited all 8 filters but in fact it only had 6)
...
# FindMCS is for 2 or more mols & returns single-fragment MCS
# RascalMCES (maximum common edge substructures) between 2 mols only & returns multi-fragment MCES (not going to use in this case)

## Below code example from https://rdkit.org/docs/GettingStartedInPython.html#findmcs with bits of my temporary draft code 
...
Draw.MolsToGridImage(mols, subImgSize=(500, 400), molsPerRow=2, legends = list(df_3a4_s_inh_p["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(res.queryMol) for m in mols])
```
...
?fragments or other things (maybe another post? as this one could be long again...)