...
A code example from https://rdkit.org/docs/GettingStartedInPython.html#findmcs might be a good starting point to learn about MCS.
...
*Below section might be replaced with the set MCS parameters to match ring section below*
...
RDKit docs on [rdkit.Chem.rdDepictor.GenerateDepictionMatching2DStructure()]
...
* When both BondCompareParameters are set to true, nil highlighted ring shown (apart from ethyl chains being highlighted in every molecule)

* Turned off CompleteRingsOnly - allowing partial rings showed highlighted MCS 

* ?similar to earlier result when using ringMatchesRingOnly = True in FindMCS() for all_s3a4_mols

<br>

**Non-ring systems**

Thinking about stereochemistry of the mols, specifically for bonds:
...
```{python}
# Consider using CompareOrderOutsideRing class (for bonds) & CompareElementsOutsideRing class (for atoms)
# to see if MCS changes (e.g. ?becomes larger)
...
*interim findings, subjected to further changes*
...
* Many pharmaceuticals have different types of rings found in their chemical structures, so it is unsurprising that inhibiting CYP3A4 is quite common in approved drugs
