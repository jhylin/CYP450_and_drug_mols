...
* https://www.blopig.com/blog/2023/06/customising-mcs-mapping-in-rdkit/ - a more detailed post providing code examples about atom matching
...
Draw.MolsToGridImage(mols_s3a4, subImgSize=(500, 400), molsPerRow=2, legends = list(df_3a4_s_inh_p["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(s3a4_mcs.queryMol) for m in mols_s3a4])
```
...
Draw.MolsToGridImage(mols_s3a4, subImgSize=(500, 400), molsPerRow=2, legends = list(df_3a4_s_inh_p["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(s3a4_mcs_80.queryMol) for m in mols_s3a4])
```
...
Draw.MolsToGridImage(mols_m3a4, subImgSize=(400, 300), molsPerRow=2, legends = list(df_3a4_s_inh_p["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(s3a4_mcs.queryMol) for m in mols_m3a4])
```

Only some of the drug molecules in this group were highlighted with MCS. It appeared to be less than the strong CYP3A4 inhibitor group.
...
Draw.MolsToGridImage(mols_all, subImgSize=(500, 400), molsPerRow=2, legends = list(df_3a4_all["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(all_3a4_mcs.queryMol) for m in mols_all])
```
...
```{python}
Draw.MolsToGridImage(mols_all, subImgSize=(500, 400), molsPerRow=2, legends = list(df_3a4_all["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(all_3a4_mcs_ring.queryMol) for m in mols_all])
```
...
Draw.MolsToGridImage(list(df_s3a4_macro["rdkit_mol"]), subImgSize=(300, 300), molsPerRow=2, legends = list(df_s3a4_macro["pref_name"]))
```
...
Draw.MolsToGridImage(list(df_s3a4_antiv["rdkit_mol"]), subImgSize=(300, 300), molsPerRow=2, legends = list(df_s3a4_antiv["pref_name"]))
```
...
Draw.MolsToGridImage(list(df_s3a4_antiv["rdkit_mol"]), subImgSize=(300, 300), molsPerRow=2, legends = list(df_s3a4_antiv["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(s3a4_antiv.queryMol) for m in mols_all])
```

Since MCS was not actually obvious here, I thought to try aligning these 5 antivirals to see if this was possible.
...
Draw.MolsToGridImage(mols_antiv, molsPerRow=2, subImgSize=(200, 200), legends=list(df_s3a4_antiv["pref_name"]))
