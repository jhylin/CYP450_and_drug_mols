...
Maximum common substructures (MCS) is something I'm interested in trying out so below are some examples of finding MCS in these CYP inhibitors.

Below are some information regarding MCS in RDKit:
...
* [TeachOpenCADD's MCS talktorial](https://projects.volkamerlab.org/teachopencadd/talktorials/T006_compound_maximum_common_substructures.html) - a nice tutorial-like piece that is good for learning as it introduces  MCS and and explains about the FMCS algorithm
...
Aromatic (and macrolide) rings are definitely involved in the MCS.

<br>

###### **All CYP3A4 inhibitors**

The moderate CYP3A4 inhibitors are added next to see if MCS changes when looking at all of the CYP3A4 inhibitors.
...
The ring bond matching also shows very similar result here where it restricts the MCS matching to ring bonds only.

<br>

**Some other interesting code re. MCS**

One of the code examples I'd like to try here, in order to see if it makes any difference from above, is the code snippets shared from a RDKit post by Paolo Tosco - "[New MCS features in 2023.09.1](https://greglandrum.github.io/rdkit-blog/posts/2023-10-27-mcswhatsnew.html)" - the "Custom Python AtomCompare and BondCompare classes" section (other 3 sections were on degenerate MCSs, FinalMatchChecker and ShouldAcceptMCS - might be useful in some other cases).
...
* When both BondCompareParameters are set to true, nil highlighted ring is shown (apart from ethyl chains being highlighted in every molecule)

* When turning off CompleteRingsOnly, this allows partial rings to be shown in highlighted MCS 

* Similar output is generated when using ringMatchesRingOnly = True in FindMCS() earlier for all CYP3A4 inhibitors 

<br>

**Non-ring systems**

Class code below are borrowed from the [RDKit blog post](https://greglandrum.github.io/rdkit-blog/posts/2023-10-27-mcswhatsnew.html), which explains about why it's done as a custom "class" code and not a "function" one.
...
# I've also had to think harder about the what the class code are doing below, especially the differences between comparing bond orders and ring atoms... I can only describe it as both a restrictive (matching non-ring bonds only) and lenient (but still comparing ring atoms) process in order to cover the non-ring parts
...
[Another code example](https://gist.github.com/iwatobipen/6d8708d8c77c615cfffbb89409be730d) from iwatobipen shows how to highlight molecular differences, which is opposite to highlighting only the MCS. Alternative URL link and more examples can be accessed via [RDKit Cookbook](https://rdkit.org/docs/Cookbook.html#highlight-molecule-differences).

Possible use cases of this code may be when dealing with a large set of analogues with changes in R-groups or during large compound screening and search (just as a few examples only, not exhaustive). The main thing I can see from the code is that it begins with finding MCS for the input molecules, then use SMARTS strings of the MCS in order to find atoms not within the MCS (using [GetSubstructMatch()](https://rdkit.org/docs/source/rdkit.Chem.rdchem.html#rdkit.Chem.rdchem.Mol.GetSubstructMatch) part) then highlight that part of the molecules.
