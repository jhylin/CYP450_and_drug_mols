...
Maximum common substructures might be interesting to look at in this group.
...
```{python}
Draw.MolsToGridImage([s_3a4_mcs1, s_3a4_mcs2], legends=["MCS1", "MCS2 with threshold = 0.8"])
```
...
Added in moderate CYP3A4 inhibitors to see if MCS changes (as above only for strong 3A4 inhibitors)
...
```{python}
mols = list(df_3a4_all["rdkit_mol"])
```

```{python}
# Find MCS for all CYP3A4 inhibitors
all_3a4_mcs = rdFMCS.FindMCS(mols)
```
...
Draw.MolsToGridImage(mols, subImgSize=(500, 400), molsPerRow=2, legends = list(df_3a4_all["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(all_3a4_mcs.queryMol) for m in mols])
```
...
all_3a4_mcs_ring = rdFMCS.FindMCS(mols, ringMatchesRingOnly=True)
```

```{python}
Draw.MolsToGridImage(mols, subImgSize=(500, 400), molsPerRow=2, legends = list(df_3a4_all["pref_name"]), highlightAtomLists=[m.GetSubstructMatch(all_3a4_mcs_ring.queryMol) for m in mols])
```
