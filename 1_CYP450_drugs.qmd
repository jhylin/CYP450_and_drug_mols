...
Draw.MolsToGridImage(
    ...
    molsPerRow=2, 
    legends = list(df_3a4_all["pref_name"]), 
    highlightAtomLists=[m.GetSubstructMatch(all_3a4_mcs_ring.queryMol) for m in mols_all]
    )
```
...
Draw.MolsToGridImage(
    ...
    molsPerRow=3, 
    legends = list(df_3a4_all["pref_name"]), 
    highlightAtomLists=[m.GetSubstructMatch(all_3a4_ringMCS.queryMol) for m in mols_all]
    )
```
...
Class code below were borrowed from RDKit blog post (https://greglandrum.github.io/rdkit-blog/posts/2023-10-27-mcswhatsnew.html), which has explained the code very well already, particularly on why this was done as a custom "class" code and not a "function" one. I just wanted to try the code to see if the MCS would change as suggested.
...
Draw.MolsToGridImage(
    ...
    molsPerRow=3, 
    legends = list(df_3a4_all["pref_name"]), 
    highlightAtomLists=[m.GetSubstructMatch(all_3a4_orMCS.queryMol) for m in mols_all]
    )
```
