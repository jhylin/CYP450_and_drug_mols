...
- Small introduction on drug toxicology and metabolism regarding cytochrome P450s (CYPs)
    - more on drug structural alerts
    - more on CYPs and ADMET
- Data extraction using chembl_downloader
- Data import and preprocessing
- Quick check on structural validites
- Further look into duplicated structures and stereochemistries
    - quinidine
    - itraconazole
- Maximum common substructures (**need to revise and shorten**)
    - CYP3A4 inhibitors
    - CYP2D6 inhibitors
...
3. **Cytochrome P450 enzymes** (CYP450, CYP isoenzymes, CYP or CYPs), the well-known ones were CYP3A4, 2D6, 1A2, 2C9 and 2C19
    ...
    CYP450 enzymes played a key role in the metabolism and toxicology parts of the ADMET process of drugs. 

    When a drug behaves like a cytochrome inhibitor, it inhibits the activity of a particular cytochrome enzyme e.g. CYP3A4 leading to a reduction of clearance of a particular therapeutic drug e.g. a CYP3A4 substrate such as apixaban, thus increasing its plasma concentration *in vivo* causing a higher chance of adverse effect (which in the context of apixaban, this means the poor person taking the apixaban may get excessive bleeding...).

    My very inital naive thought was that if we could at least cover some of the drug toxicology part during drug design and discovery process, this might be able to save some resources along the way (obviously it won't be this simple...).
    
    Nevertheless, it might still be useful and interesting to look into the relationship between CYP450 and small drug molecules to see if there were anything worth further explorations. This post would start with the two largest groups of CYP inhibitors, so focussing on CYP3A4 and 2D6 first.
...
I was only really curious about the data sources  used to build these ChEMBL structural alerts, so below were some of my notes on these sources.
...
From ChEMBL 20 to 23, there were 8 filters in total (agreed with rd_filters' README.md that there weren't many documentations about this in ChEMBL, because I did try to search for this on ChEMBL but nothing in details were found), the sources of the two additional ones were as follow:
...
RDKit (section on ["Filtering unwanted substructures"](https://www.rdkit.org/docs/GettingStartedInPython.html#filtering-unwanted-substructures)) also has another NIH filter based on two other references [@Jadhav2010] and [@Doveston2015]. At one point I was so confused with this NIH filter here and the NIH MLSMR one above... I thought they were the same but when I looked further they cited different papers actually. 

RDKit also uses the above 8 filters mentioned in ChEMBL in its [FilterCatalogs](https://www.rdkit.org/docs/source/rdkit.Chem.rdfiltercatalog.html#rdkit.Chem.rdfiltercatalog.FilterCatalogParams.FilterCatalogs) class currently. Brenk filter seems to be the same as the CHEMBL_Dundee one since both of them quoted the same journal paper as reference. It's also got a ZINC one I think. 

Before I get very carried away, I'll stop searching for every structural alerts papers here as there are many in the literatures.

<br>

###### **More on CYPs and ADMET**

A bit of a sidetrack for this part (feel free to skip) as I came across a new paper online recently about using deep learning model for ADMET prediction and the data source used by this paper was from Therapeutics data commons (TDC). So while I was working on this relevant topic of CYP, metabolism and toxicology of the ADMET, I thought to dig a bit deeper to see what sort of data were used by TDC.
...
I'm using SQL via chembl_downloader again to download approved drugs with their ChEMBL ID and equivalent canonical SMILES. All of the CYP3A4 and 2D6 inhibitors extracted from ChEMBL are based on the [Flockhart table of drug interactions](https://drug-interactions.medicine.iu.edu/MainTable.aspx) [@Flockhart2021].

Note: Three other categories of medicines were not going to be looked at for now, which were the weak inhibitors, ones with *in vitro* evidence only and ones that were still pending reviews.

A bit about retrieving data here, the following might not be the best way to get the data, but I've somehow incorporated chembl_downloader into my own small piece of function code to retrieve SMILES of approved drugs. Earlier on, I was repeating SQL queries and thought to improve them by using a function code instead (see Python script named as "cyp_drugs.py" in the [repo](https://github.com/jhylin/CYP450_and_drug_mols)). It removed a large chunk of SQL query string every time when trying to get different CYP inhibitors, leaving only approved drug names so it might be easier to read and understand.

Another possible way was to use get_target_sql(), e.g. using a specific CYP enzyme as the protein target to retrieve data, but it appeared that there were no clear data marked to indicate the potency of CYP inhibition or induction (i.e. weak, moderate or strong) in the ChEMBL database (an [example link for CYP2D6 in ChEMBL](https://www.ebi.ac.uk/chembl/web_components/explore/activities/STATE_ID:pxs_ydUxNelxmAa9ckYEPw%3D%3D)). The Flockhart table has clearly annotated each approved drug with journal paper citations so I decided to stick with the previous method.
...
Initially, four categories of approved drugs were retrieved - both strong and moderate CYP3A4 inhibitors, and also the strong and moderate CYP2D6 inhibitors. CYP3A4 inhibitors were the largest cohort of all the cytochrome inhibitors known so far (based on clinical documentations). 
...
```{python}
## Tried using pandas 2.2.2, numpy 2.0.0 and rdkit 2024.3.1 (all latest major versions, 
...
## Eventually used downgraded versions of pandas and numpy instead 
# - confirmed that pandas 2.1.4, numpy 1.26.4 & rdkit 2024.3.1 worked with no error messages generated as output
...
Main thing was to check structural validities.
...
I had a feeling looking into stereochemistries might not be needed for now but as a fun exercise, I should, so below was a small section on looking at two drugs with duplicated entries more closely with their stereochemistries.

<br>

###### **quinidine**

Different stereochemistries between the two quinidines were spotted.
...
Quinidine had 4 defined atom stereocentre count as per PubChem compound summary (as one of possible references for cross-checking) - this was based on the calculation for [CHEMBL1294](https://pubchem.ncbi.nlm.nih.gov/compound/441074#section=Computed-Properties&fullscreen=true), which was the same as the quinidine spotted at index row 4.

Dropping the quinidine at index row 3 for now.

```{python}
# Note: old index unchanged for now (re-index later if needed)
...
Two itraconazoles were found to check for stereochemistries.
...
Clearly, even if the SMILES of these two itraconzoles were not converted into RDKit molecules, we could probably tell one of them had stereochemistries and the other molecule was without due to the presence of "@" in the SMILES string for the one at index row 4. So the output images showed exactly that - one with chiral centres, where the other did not have any.

PubChem calculations however generated different result for itraconazole. It seemed it only had one defined atom stereocentre count and two undefined stereocentre counts ([PubChem reference](https://pubchem.ncbi.nlm.nih.gov/compound/44428219#section=Computed-Properties&fullscreen=true)).

I've also noted that the two itraconzoles obtained from ChEMBL had different ChEMBL ID numbers (ChEMBL ID: CHEMBL22587 and CHEMBL64391) to the one calculated in PubChem (ChEMBL ID: CHEMBL224725). So below I've looked into CHEMBL224725 first.
...
Well, I've just found out that if I searched for "itraconazole" directly in ChEMBL, only four entries appeared with ChEMBL IDs of CHEMBL64391, CHEMBL22587, CHEMBL882 and CHEMBL5090785, and there was no ChEMBL224725. Then I noticed a spelling error (which was most likely a typo by accident) of itraconazole as "itraconzole", which was actually carried over into PubChem as well. I did double check to make sure both "itraconzole" and the usual itraconazole were referring to the same chemical structure. Below were some screenshots showing the typo. ![Screenshot of "itraconzole" in ChEMBL database](chembl_itracon.png) ![Screenshot of "itraconzole" in PubChem that was inherited from ChEMBL](pubchem_itracon.png) 

So to add this likely-mis-spelled "itraconzole" into the dataframe, I literally added it into the SQL query above when obtaining drug information through chembl_downloader.
...
There was only one stereocentre for "itraconzole", which would match the CHEMBL224725 entry for "itraconzole" in PubChem. Without looking into other cross-referencing sources, and if only sticking with PubChem for now, I then went back to check all 3 (stereochemically-)different versions of itraconazole and found that the RDKit stereochemical calculations of these 3 itraconazoles had all basically matched their equivalent PubChem computations for atom sterecentre counts.
...
Keeping the one with max phase marked as 4.0 (the other two had "NaN" and had no relevant medical or therapeutic indications data documented in PubChem).
...
Maximum common substructures (MCS) might be interesting to look at for these CYP inhibitors.

Regarding MCS in RDKit:

* [FindMCS](https://rdkit.org/docs/source/rdkit.Chem.rdFMCS.html) was for 2 or more mols and returns single-fragment MCS - based on FMCS algorithm [@dalke2013]

* RascalMCES (maximum common edge substructures) was for 2 mols only and returns multi-fragment MCES. A [RDKit blog post](https://greglandrum.github.io/rdkit-blog/posts/2023-11-08-introducingrascalmces.html) by Dave Cosgrove talked about this in more details

Some code examples to refer to:

* A [RDKit code example](https://rdkit.org/docs/GettingStartedInPython.html#findmcs) might be a good starting point to learn about MCS.

* [TeachOpenCADD's MCS talktorial](https://projects.volkamerlab.org/teachopencadd/talktorials/T006_compound_maximum_common_substructures.html) - a nice tutorial-like piece that was good for learning as it introduced and explained MCS and FMCS algorithm

* ["Customising MCS mapping in rdkit"](https://www.blopig.com/blog/2023/06/customising-mcs-mapping-in-rdkit/) - a more detailed post providing code examples about atom matching

<br>

###### **Strong CYP3A4 inhibitors**

*To revise and shorten*
...
```{python}
# Get number of atoms and bonds in MCS
s3a4_mcs.numAtoms, s3a4_mcs.numBonds
```

```{python}
# Generate SMARTS string for the MCS
s3a4_mcs.smartsString
```

One way to customise MCS was via reducing molecule threshold to relax the MCS rule as suggested by the TeachOpenCADD reference above.
...
It appeared the MCS for most of them involved (partial) rings (e.g. cycloalkane, aromatic and macrolide ones), so ring bonds matching will be looked at next.
...
*Shorten/remove this part*

There were 3 antifungals, 2 macrolides, about 5 antivirals and 4 kinase inhibitors (e.g. ceritinib, tucatinib, idelalisib, ribociclib) in the strong CYP3A4 inhibitor cohort. For people unfamiliar with these drugs, ChEMBL database do have "drug indications" recorded for each approved drug that you might be able to use to deduce what each drug was used for. The "drug indications" could be added into the SQL query at the beginning so that it'd be included into the dataframes (my old role happened to help here, so I kind of didn't need to do this).
...
Just to clarify, telithromycin was actually a [ketolide antibacterial](https://www.accessdata.fda.gov/drugsatfda_docs/label/2015/021144s019lbl.pdf#page=1), which was very similar to macrolide antibiotics (clarithromycin in this case).

```{python}
# Just showing how similar clarithromycin & telithromycin were 
Draw.MolsToGridImage(
...
Draw.MolsToGridImage(
    ...
    molsPerRow=2, 
    legends = list(df_s3a4_antiv["pref_name"]), 
    highlightAtomLists=[m.GetSubstructMatch(s3a4_antiv.queryMol) for m in list(df_s3a4_antiv["rdkit_mol"])]
    )
```
...
* [Code example from RDKit docs](https://www.rdkit.org/docs/GettingStartedInPython.html#working-with-2d-molecules-generating-depictions)

* RDKit docs on [rdkit.Chem.rdDepictor.GenerateDepictionMatching2DStructure()](https://www.rdkit.org/docs/source/rdkit.Chem.rdDepictor.html#rdkit.Chem.rdDepictor.GenerateDepictionMatching2DStructure)

If trying to align these 5 antivirals, it would fail the first time and if I turned on the "acceptFailure" mode then it would be like this below. Ideally, it should be used on a series of analogues or SAR analysis (probably not to be used this way, but I was just curious).
...
for mol in mols_antiv:
    _ = AllChem.GenerateDepictionMatching2DStructure(mol,temp, acceptFailure = True)
...
Draw.MolsToGridImage(
    ...
    subImgSize=(400, 300), 
    molsPerRow=2, 
    legends = list(df_s3a4_ki["pref_name"]), 
    highlightAtomLists=[m.GetSubstructMatch(s3a4_ki.queryMol) for m in list(df_s3a4_ki["rdkit_mol"])]
    )
```

Pyrimidine rings (or part of fused pyrimidine derivatives in some cases) were shown in MCS highlighting for all 4 kinase inhibitors.

<br>

###### **Back to all CYP3A4 inhibitors again**

Likely the last thing to try here as the idea came from a RDKit post by Paolo Tosco on, "[New MCS features in 2023.09.1](https://greglandrum.github.io/rdkit-blog/posts/2023-10-27-mcswhatsnew.html)" - the "Custom Python AtomCompare and BondCompare classes" section (other 3 sections were on degenerate MCSs, FinalMatchChecker and ShouldAcceptMCS - might be useful in some other cases).

Some notes from me:

* AtomCompare & BondCompare - looking at elements/bond orders/aromaticities in ring systems
...
Since it appeared that the MCS found above for all (moderate & strong) CYP3A4 inhibitors involved rings quite a bit, I might try the second option first to see what other MCS might exist in non-ring systems.

<br>

**Ring systems**

However, before jumping to the non-ring systems, I just wanted to try the AtomCompare & BondCompare along with RingMatchesRingOnly + CompleteRingsOnly first.
...
* When both BondCompareParameters were set to true, nil highlighted ring was shown (apart from ethyl chains being highlighted in every molecule)

* When turning off CompleteRingsOnly, this allowed partial rings to be shown in highlighted MCS 

* Similar output was generated when using ringMatchesRingOnly = True in FindMCS() earlier for all CYP3A4 inhibitors 

<br>

**Non-ring systems**

Class code below were borrowed from the [RDKit blog post](https://greglandrum.github.io/rdkit-blog/posts/2023-10-27-mcswhatsnew.html), which explained about why it was done as a custom "class" code and not a "function" one. I just wanted to try the code to see if the MCS would change as suggested.

```{python}
# I've attempted to add comments to the code below (hopefully not too far off from what they do actually)

# I had to think harder about the what the class code were doing below, especially the differences between comparing bond orders and ring atoms... I could only describe it as both a restrictive (matching non-ring bonds only) and lenient (but still comparing ring atoms) process in order to cover the non-ring parts
...
By using the suggested class code above, the MCS has indeed become larger, where a full ring was highlighted as the MCS.

<br>

**Some other interesting code re. MCS**

[A code example](https://gist.github.com/iwatobipen/6d8708d8c77c615cfffbb89409be730d) from iwatobipen showed how to highlight molecular differences, which was opposite to highlighting only the MCS. Alternative URL link and more examples could be accessed via [RDKit Cookbook](https://rdkit.org/docs/Cookbook.html#highlight-molecule-differences).

Possible use cases of this code might be when dealing with a large set of analogues with changes in R-groups or during large compound screening and search (just as a few examples only, not exhaustive). The main thing I could see from the code was that it began with finding the MCS for the input molecules, then use SMARTS strings of the MCS in order to find atoms not within the MCS ([GetSubstructMatch()](https://rdkit.org/docs/source/rdkit.Chem.rdchem.html#rdkit.Chem.rdchem.Mol.GetSubstructMatch) part) then highlight that part of the molecules.
...
```{python}
# Get list of RDKit mols
mols_s2d6 = list(df_2d6_s_inh_p["rdkit_mol"])

# Find MCS in mols
s2d6_mcs = rdFMCS.FindMCS(mols_s2d6)
...
Draw.MolsToGridImage(
    ...
    molsPerRow=2, 
    legends = list(df_2d6_s_inh_p["pref_name"]), 
    highlightAtomLists=[m.GetSubstructMatch(s2d6_mcs.queryMol) for m in mols_s2d6]
    )
```

Ring with longer alkyl chain appeared to be in the MCS (as a very general first look).
...
```{python}
# Get list of RDKit mols
mols_m2d6 = list(df_2d6_m_inh_p["rdkit_mol"])

# Find MCS in mols
m2d6_mcs = rdFMCS.FindMCS(mols_m2d6)
...
Draw.MolsToGridImage(
    ...
    molsPerRow=2, 
    legends = list(df_2d6_m_inh_p["pref_name"]), 
    highlightAtomLists=[m.GetSubstructMatch(m2d6_mcs.queryMol) for m in mols_m2d6]
    )
```

Both aromatic and non-aromatic rings (fused ones also included) were shown here as MCS for moderate CYP2D6 inhibitors.
...
```{python}
# Concatenate dfs for moderate & strong CYP2D6 inhibitors
...
If combining both moderate and strong CYP2D6 inhibitors, without using further MCS functionalities, only ring structures were shown in the MCS highlighting.

One characteristic about the CYP2D6 inhibitors here was that some of them were drugs acting on or relating to central nervous system (about 9 out of 14 drugs) - e.g. bupropion, fluoxetine, paroxetine, clobazam, doxepin, duloxetine, locaserin (an approved drug but withdrawn due to adverse effect; acting on serotonin receptor), moclobemide and rolapitant (antiemetic that crosses blood-brain barrier).

<br>

##### **Some findings and future work**

This post has become too long already so it would end here. Below were some small findings and thoughts.

* Partial rings within heterocycles were shown mainly in MCS of CYP3A4 inhibitors (clinically-documented moderate and strong CYP3A4 inhibitors only) if we only used the default rdFMCS.AtomCompare and rdFMCS.BondCompare with the ring matching parameters (e.g. RingMatchesRingOnly)

* Full rings were shown in the MCS if we extended the bond and atom comparisons to cover the non-ring systems (i.e. a more flexible comparison than a restricted one, but still making the code to match rings only) 

* Many pharmaceuticals had different types of rings found in their chemical structures [@Taylor2014], so it was unsurprising that inhibiting CYP3A4 was commonly found in approved drugs

* It might be interesting to compare the MCS found in CYP3A4 inhibitors with the MCS for CYP3A4 substrates

* It appeared that the difference between the MCS of the moderate and strong CYP2D6 inhibitors was the alkyl chain attached to ring structures (although partial ring highlighted only) in strong CYP2D6 inhibitors, which was not found in the moderate CYP2D6 inhibitors

* As to the differences between CYP3A4 and 2D6 inhibitors, perhaps it was very subtle, as both groups seemed to show ring structures... Perhaps looking at CYP2D6 substrates will help (I was only scratching the surface here maybe)

* It might also worth to spend more time on exploring the CYP structures in relation to these inhibitors (more of a structure-based drug design realm), which might reveal more insights than what were found here
...
Thanks to every contributor, developer or authors of every software package used in this post, and also the online communities that are behind them. Also, before I forget, the thanks would also be extended to everyone who was involved in the relevant research journal papers mentioned in this post.