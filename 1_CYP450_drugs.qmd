...
Aromatic (and macrolide) rings are highlighted in the MCS in this group.

<br>

###### **All CYP3A4 inhibitors**

The moderate CYP3A4 inhibitors are then added next in order to see if MCS changes when looking at all the CYP3A4 inhibitors.
...
I'll also attempt to add some code comments below to explain how the code works (anyone's welcomed to report any issues for this part or post for updates or changes).
...
The class code below is also borrowed from the [RDKit blog post](https://greglandrum.github.io/rdkit-blog/posts/2023-10-27-mcswhatsnew.html), which explains about why it's done as a custom "class" code and not a "function" one.

```{python}
#| code-fold: true

# I've had to think harder about the what the class code are doing below, especially the differences between comparing bond orders and ring atoms... I can only describe it as both a restrictive (matching non-ring bonds only) and lenient (but still comparing ring atoms) process in order to cover the non-ring parts (?)
...
[A second code example](https://gist.github.com/iwatobipen/6d8708d8c77c615cfffbb89409be730d) from iwatobipen is to highlight molecular differences, which is different from highlighting only the MCSes. Alternative URL link and more examples can be accessed via [RDKit Cookbook](https://rdkit.org/docs/Cookbook.html#highlight-molecule-differences).

Possible use cases of this code may be when dealing with a large set of analogues with changes in R-groups or during large compound screening and search (just as a few examples only). The main thing I can see from the code is that it begins with finding MCS for the input molecules, then use SMARTS strings of the MCS in order to find atoms not within the MCS (using [GetSubstructMatch()](https://rdkit.org/docs/source/rdkit.Chem.rdchem.html#rdkit.Chem.rdchem.Mol.GetSubstructMatch) part) then highlight that part of the molecules.

<br>

###### **All CYP2D6 inhibitors**

Because of how the MCSes have turned out for CYP3A4 inhibitors above, I think I should just look at CYP2D6 inhibitors as a whole here. First thing is to combine the dataframes together, between the moderate and strong inhibitor groups.
...
Again, ring structures are the MCS in the highlighted parts.
...
* Many pharmaceuticals have different types of rings found in their chemical structures [@Taylor2014], so it is unsurprising that the MCSes found from these compounds are mainly rings

* It may be more interesting to compare the MCSes between CYP3A4 and CYP2D6 substrates

* Looking at CYP structures in relation to these inhibitors may also reveal more insights about possible binding sites or mechanisms of actions for these compounds. For example, some of CYP2D6 inhibitors are drugs acting on central nervous system (about 9 out of 14 drugs) - e.g. bupropion, fluoxetine, paroxetine, clobazam, doxepin, duloxetine, locaserin (an approved drug but withdrawn due to adverse effect; acting on serotonin receptor), moclobemide and rolapitant (antiemetic that crosses blood-brain barrier). For the CYP3A4 inhibitors, they are also in several different classes of drugs e.g. antivirals, antifungals, kinase inhibitors and so on.

* Future posts may involve looking at CYP substrates (other CYP inhibitors may also be looked at but most likely may lead to similar result as this one... however it may just be nice to confirm it)
...
Thanks to every contributor, developer or authors of every software package used in this post, and also the online communities that are behind them. Also, before I forget, the thanks would also be extended to everyone who was involved in the relevant research journal papers mentioned in this post. Lastly, special thanks to Noel O'Boyle for being very patient with reading my awfully-long old draft earlier and pointed out some useful things to change (I kind of got lost when writing the draft... due to it being a "free-style post", I should avoid doing this again).